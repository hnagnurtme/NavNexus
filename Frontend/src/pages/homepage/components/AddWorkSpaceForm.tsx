...
// Default export component (ready to paste into a project using Tailwind)
export default function AddWorkSpaceForm({ onCreate, onCancel }: Props) {
  ...
  function handleFilesChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (!list) return;
    setFiles(Array.from(list).slice(0, 10)); // limit to 10 files in UI
  }
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      // best-effort error handling; bubble up or show toast in real app
    ...
    >
      <div className="flex items-start gap-6">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-[#03C75A]">Create a new workspace</h3>
          <p className="text-sm text-[#b3b3b3] mt-1">A workspace keeps documents, notes and insights together.</p>

          <div className="mt-6 space-y-4">
            <div>
              <label className="block text-xs font-medium text-[#bfcfc3]">Workspace name</label>
              <input
                value={name}
            ...
            <div>
              <label className="block text-xs font-medium text-[#bfcfc3]">Description</label>
              <textarea
                value={description}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label className="block text-xs font-medium text-[#bfcfc3]">Visibility</label>
                <div className="flex gap-2 mt-2">
                  {(['private', 'team', 'public'] as WorkspacePayload['visibility'][]).map((v) => (
                    <button
                      key={v}
                      type="button"
              ...
              <div>
                <label className="block text-xs font-medium text-[#bfcfc3]">Accent color</label>
                    ...
                    {['#03C75A', '#00A84D', '#94b49f', '#2f3b2f'].map((sw) => (
                      <button
                        ...
                        className={`h-8 w-8 rounded-full border ${
                          color === sw ? 'ring-2 ring-[#03C75A]/60' : 'border-[#222]'
                        }`}
            ...
            <div>
              <label className="block text-xs font-medium text-[#bfcfc3]">Upload documents (optional)</label>
              <div className="flex items-center gap-3 mt-2">
                <input
                  ref={fileRef}
                ...
                <div className="text-xs text-[#b3b3b3]">Max 10 files · PDF, DOCX, TXT</div>
              </div>

              {!!files.length && (
                <div className="grid gap-2 mt-3">
                  {files.map((f, i) => (
                    <div key={i} className="flex items-center justify-between gap-3 rounded-md bg-[#0b0b0b] p-2 border border-[#222]">
                      <div className="text-sm truncate">{f.name}</div>
                      <div className="flex items-center gap-2">
                        <div className="text-xs text-[#888]">{Math.round(f.size / 1024)} KB</div>
                        <button
                          type="button"
        ...
        <aside className="w-80 shrink-0">
          <div className="rounded-xl border border-[#222]  p-4" style={{ background: `linear-gradient(180deg, ${color}22, #0b0b0b)` }}>
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-lg" style={{ backgroundColor: color }} />
              <div>
                <div className="text-sm font-semibold text-[#f5f5f5]">{name || 'Workspace name'}</div>
                <div className="text-xs text-[#b3b3b3]">{visibility} workspace</div>
              </div>
            </div>

            <p className="mt-3 text-xs text-[#cfcfcf] truncate">{description || 'A short workspace summary will appear here.'}</p>

            <div className="mt-4 flex items-center gap-2 text-xs text-[#b3b3b3]">
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[#03C75A]/20 text-[#03C75A] font-semibold">{files.length}</div>
              <div>{files.length} docs</div>
            </div>

            <div className="mt-4 text-[11px] text-[#9aa69a]">Preview updates as you type. Accent color is applied to the card and CTA.</div>
          </div>
        </aside>
      </div>
    </form>
